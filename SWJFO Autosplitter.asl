...
state("starwarsjedifallenorder")
{
    ...
    byte LRT2: 0x36E7200;
    int sceneChange: 0x036D8830, 0x8, 0x138, 0x180;
}
...
start{
    if(current.LRT == 1 && current.LRT2 == 0 && current.sceneChange != 1){
        return true;
    } else{
        return false;
    }
...
/*split
{
    if(((current.Bracca == 64 || current.Bracca == 66) && current.playerPos > -307352 && current.playerPos < -307350 && vars.split == 0)//Bracca 1
    ||
    ...
    ||
    (current.playerPos > 8130 && current.playerPos < 8202 && current.playerPos2 > 3970 && current.playerPos2 < 4020 && current.Bracca == 129 && current.mapBYTE == 216 && vars.split == 0))//Nur 7
    { 
        vars.split++;
        return true;
    }
